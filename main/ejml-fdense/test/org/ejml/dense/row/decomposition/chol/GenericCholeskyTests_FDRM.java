...
@Generated("org.ejml.dense.row.decomposition.chol.GenericCholeskyTests_DDRM")
public abstract class GenericCholeskyTests_FDRM extends EjmlStandardJUnit {
    boolean canL = true;
    boolean canR = true;
    ...
    @Test void testDecomposeR() {
        ...
        var A = new FMatrixRMaj(3, 3, true, 1, 2, 4, 2, 13, 23, 4, 23, 90);
        var R = new FMatrixRMaj(3, 3, true, 1, 2, 4, 0, 3, 5, 0, 0, 7);
    ...
    @Test void zeroShapeMatrix() {
        var A = new FMatrixRMaj(0, 0);

        if (canL) {
            CholeskyDecomposition_F32<FMatrixRMaj> cholesky = create(true);
            assertTrue(cholesky.decompose(A));
            FMatrixRMaj found = cholesky.getT(null);
