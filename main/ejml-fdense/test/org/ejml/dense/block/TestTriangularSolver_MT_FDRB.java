...
@Generated("org.ejml.dense.block.TestTriangularSolver_MT_DDRB")
class TestTriangularSolver_MT_FDRB extends EjmlStandardJUnit {
	@Test
	void invert_two() {
		// block size
		int r = 3;
		...
		for (int size = 1; size <= 20; size+=r+1) {
			...
			FMatrixRBlock T_inv = T.createLike();

			// Invert and save results into a difference matrix
			TriangularSolver_MT_FDRB.invert(r, false, new FSubmatrixD1(T), new FSubmatrixD1(T_inv), null);
...
//                System.out.println("triangle "+triangleSize+" cols "+cols);
	...
	private void checkSolve( FMatrixRBlock T, FMatrixRBlock B, FMatrixRBlock Y,
		...
		TriangularSolver_MT_FDRB.solve(r, upper, new FSubmatrixD1(T), new FSubmatrixD1(Y), transT);
	...
	private void checkSolveUnaligned( FMatrixRBlock T, FMatrixRBlock B, FMatrixRBlock Y,
		...
		// Y is overwritten with the solution
		TriangularSolver_MT_FDRB.solve(r, upper, new FSubmatrixD1(T2, 0, size, 0, size), new FSubmatrixD1(Y), transT);
...
//        sub_B.original.print();
		...
		TriangularSolver_MT_FDRB.solveBlock(3, !solveL, sub_L, sub_B, transT, transB);
	...
	private void makeSolvable( FMatrixRBlock T ) {
		// Attempt to ensure it's a numerically stable invertible matrix
		for (int i = 0; i < T.numCols; i++) {
			T.set(i,i, 1.0f + rand.nextFloat());
		}
	}
}
