...
@Generated("org.ejml.sparse.csc.TestCommonOps_DSCC")
public class TestCommonOps_FSCC extends EjmlStandardJUnit {
    @Test
    void checkShape() {
    ...
    public void checkIndicesSorted() {
        ...
        FMatrixSparseCSC a = FConvertMatrixStruct.convert(orig, (FMatrixSparseCSC)null);
    ...
    private void check_s_s_mult( FMatrixSparseCSC A, FMatrixSparseCSC B, FMatrixSparseCSC C, boolean exception ) {
        ...
        try {
            ...
            FMatrixRMaj denseB = FConvertMatrixStruct.convert(B, (FMatrixRMaj)null);
            FMatrixRMaj expected = new FMatrixRMaj(A.numRows, B.numCols);
            CommonOps_FDRM.mult(denseA, denseB, expected);
            FMatrixRMaj found = FConvertMatrixStruct.convert(C, (FMatrixRMaj)null);
                        ...
                        if (add) {
                            if (transA) {
                                if (transB) {
                                    ...
                                    CommonOps_FSCC.multAddTransA(A_t, B, C, null);
                                    CommonOps_FDRM.multAddTransA(denseA_t, B, expected);
                                }
                            } else if (transB) {
                                CommonOps_FSCC.multAddTransB(A, B_t, C, null);
                            ...
                            if (transA) {
                                if (transB) {
                                    ...
                                    CommonOps_FSCC.multTransA(A_t, B, C, null);
                                    CommonOps_FDRM.multTransA(denseA_t, B, expected);
                                }
                            } else if (transB) {
                                CommonOps_FSCC.multTransB(A, B_t, C, null);
    ...
    public void add_empty_columns() {
        ...
        FMatrixSparseCSC A = FConvertMatrixStruct.convert(trip_A, (FMatrixSparseCSC)null);
        FMatrixSparseCSC B = FConvertMatrixStruct.convert(trip_B, (FMatrixSparseCSC)null);
    ...
    public void scale() {
        ...
        for (int length : new int[]{0, 2, 6, 15, 30}) {
            ...
            FMatrixRMaj found = FConvertMatrixStruct.convert(B, (FMatrixRMaj)null);
    ...
    public void elementMaxAbs() {
        for (int length : new int[]{0, 2, 6, 15, 30}) {
            ...
            float expected = CommonOps_FDRM.elementMaxAbs(FConvertMatrixStruct.convert(A, (FMatrixRMaj)null));
    ...
    public void elementMin() {
        for (int length : new int[]{0, 2, 6, 15, 30}) {
            ...
            float expected = CommonOps_FDRM.elementMin(FConvertMatrixStruct.convert(A, (FMatrixRMaj)null));
    ...
    public void elementMax() {
        for (int length : new int[]{0, 2, 6, 15, 30}) {
            ...
            float expected = CommonOps_FDRM.elementMax(FConvertMatrixStruct.convert(A, (FMatrixRMaj)null));
...
//            System.out.println("------------------- Trial "+trial);
            ...
            FMatrixSparseCSC A = RandomMatrices_FSCC.rectangle(rows, cols, nz_a, rand);
            FMatrixRMaj values = new FMatrixRMaj(1, 1);

            CommonOps_FSCC.maxAbsCols(A, values);
            ...
            for (int j = 0; j < cols; j++) {
                ...
                assertEquals(expected, values.get(j), UtilEjml.TEST_F32);
            }
        }
    }
...
//            System.out.println("------------------- Trial "+trial);
    ...
    public void permutationMatrix() {
        ...
        FMatrixRMaj found = FConvertMatrixStruct.convert(P, (FMatrixRMaj)null);
        ...
        found = FConvertMatrixStruct.convert(P, (FMatrixRMaj)null);
    ...
    public void dotInnerColumns() {
        ...
        float expected = ImplMultiplication_FSCC.dotInnerColumns(A, 1, B, 3, null, null);
    ...
    private void solve_dense( int m, int n ) {
        ...
        FMatrixRMaj expected = RandomMatrices_FDRM.rectangle(n, 4, rand);
    ...
    public void invert() {
        for (int m : new int[]{1, 2, 5, 15, 40}) {
            ...
            FConvertMatrixStruct.convert(A, Ad);
    ...
    public void duplicatesAdd() {
        int N = 10;
        FMatrixSparseTriplet triplet = RandomMatrices_FSTL.uniform(N, N, (N*N)/2, -1, 1, rand);
        FMatrixSparseCSC A = FConvertMatrixStruct.convert(triplet, (FMatrixSparseCSC)null);
        FMatrixSparseCSC B = A.copy();

        // first pass there should be no change
        CommonOps_FSCC.duplicatesAdd(B, null);
        assertTrue(CommonOps_FSCC.checkStructure(B));
        A.sortIndices(null);
        B.sortIndices(null);
        ...
        for (int j = 0; j < nz; j++) {
            ...
            float value = triplet.nz_value.data[j];
            triplet.addItem(row, col, value);
        }
        FConvertMatrixStruct.convert(triplet, B);
        // Remove duplicates and see if B has elements that are twice the size of elements in A
        CommonOps_FSCC.duplicatesAdd(B, null);
        assertTrue(CommonOps_FSCC.checkStructure(B));
        CommonOps_FSCC.divide(B, 2.0f, B);
        B.sortIndices(null);
        assertTrue(MatrixFeatures_FSCC.isEquals(A, B, UtilEjml.TEST_F32));
    }
    ...
    public void applyRowWise() {
        ...
        FOperatorBinaryIdx applyFunc = ( row, val ) -> val/v[row];
        FMatrixSparseCSC B = CommonOps_FSCC.applyRowIdx(A, applyFunc, null);
    ...
    public void applyColumnWise() {
        ...
        FOperatorBinaryIdx applyFunc = ( row, val ) -> val/v[row];
        FMatrixSparseCSC B = CommonOps_FSCC.applyColumnIdx(A, applyFunc, null);
    ...
    public void reduceScalar() {
        FMatrixSparseCSC A = RandomMatrices_FSCC.rectangle(10, 10, 20, rand);
        float result = CommonOps_FSCC.reduceScalar(A, 0, Float::sum);
    ...
    public void reduceColumnWise() {
        ...
        FMatrixRMaj result = CommonOps_FSCC.reduceColumnWise(A, 0, Float::sum, null, null);
    ...
    public void maskedReduceColumnWise() {
        ...
        Mask mask = FMaskFactory.builder(RandomMatrices_FSCC.rectangle(10, 1, 5, new Random(42)), false).build();
        FMatrixRMaj unmasked = CommonOps_FSCC.reduceColumnWise(A, 0, Float::sum, null, null);
        FMatrixRMaj masked = CommonOps_FSCC.reduceColumnWise(A, 0, Float::sum, null, mask);
    ...
    public void reduceRowWise() {
        ...
        FMatrixRMaj result = CommonOps_FSCC.reduceRowWise(A, 0, Float::sum, null);
