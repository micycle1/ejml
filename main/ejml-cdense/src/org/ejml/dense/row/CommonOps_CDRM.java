...
public class CommonOps_CDRM {
     ...
     */
    public static FMatrixRMaj real( CMatrixD1 input, @Nullable FMatrixRMaj output ) {
     ...
     * @param output real matrix. Modified.
     */
    public static FMatrixRMaj imaginary( CMatrixD1 input, @Nullable FMatrixRMaj output ) {
        output = UtilEjml.reshapeOrDeclare(output, input.numRows, input.numCols);
     ...
     * @param output real matrix. Modified.
     */
    public static FMatrixRMaj magnitude( CMatrixD1 input, @Nullable FMatrixRMaj output ) {
        output = UtilEjml.reshapeOrDeclare(output, input.numRows, input.numCols);
     ...
     */
    public static Complex_F32 trace( CMatrixRMaj input, @Nullable Complex_F32 output ) {
        if (output == null)
     ...
     * @param output Where the results of the operation are stored. Modified.
     */
    public static CMatrixRMaj elementMultiply( CMatrixD1 inputA, CMatrixD1 inputB, @Nullable CMatrixRMaj output ) {
        ...
        for (int i = 0; i < N; i += 2) {
            float realA = inputA.data[i];
            float imagA = inputA.data[i + 1];
            float realB = inputB.data[i];
            float imagB = inputB.data[i + 1];
     ...
     * @param output Where the results of the operation are stored. Modified.
     */
    public static CMatrixRMaj elementDivide( CMatrixD1 inputA, CMatrixD1 inputB, @Nullable CMatrixRMaj output ) {
        UtilEjml.checkSameShape(inputA, inputB, true);
        output = UtilEjml.reshapeOrDeclare(output, inputA.numRows, inputA.numCols);
        ...
        for (int i = 0; i < N; i += 2) {
            ...
            float norm = realB*realB + imagB*imagB;
     ...
     * @param output Where the results of the operation are stored. Modified.
     */
    public static CMatrixRMaj elementPower( CMatrixD1 input, float b, @Nullable CMatrixRMaj output ) {
        output = UtilEjml.reshapeOrDeclare(output, input.numRows, input.numCols);

        int N = input.getDataLength();
        for (int i = 0; i < N; i += 2) {
            float real = input.data[i];
            float imag = input.data[i + 1];

            // convert to polar form
            float r = (float)Math.sqrt(real*real + imag*imag);
            float theta = (float)Math.atan2(imag, real);

            // apply the power factor
            r = (float)Math.pow(r, b);
            theta = theta*b;

            // convert back to standard notation
            output.data[i] = r*(float)Math.cos(theta);
            output.data[i + 1] = r*(float)Math.sin(theta);
        }
     ...
     */
    public static Complex_F32 elementSum( CMatrixD1 input, @Nullable Complex_F32 output ) {
        if (output == null)
            output = new Complex_F32();
